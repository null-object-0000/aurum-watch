  ...
  React.useEffect(() => {
    ...
    chart.setOption({
      ...
      series: [{
        ...
        max: 100,
        splitNumber: 4,
        axisLine: { lineStyle: { width: 18, color: [[0.35, "#31b978"], [0.5, "#6d7683"], [1, "#d94b55"]] } },
        pointer: { show: false },
        progress: { show: true, width: 18 },
        detail: { valueAnimation: true, formatter: "{value}", color: data.sentiment.score >= 0 ? "#d94b55" : "#31b978", fontSize: 36, offsetCenter: [0, "10%"] },
        axisTick: { show: false },
  ...
  return (
    <div>
      ...
      <div className="sentiment-stats">
        <span>利多强度 <b className="red">+{data.sentiment.bullish}</b></span>
        <span>利空强度 <b className="green">{data.sentiment.bearish}</b></span>
        <span>中性占比 <b>{data.sentiment.neutralShare}%</b></span>
      </div>
...
export function EventFeed({ events }: { events: NewsEvent[] }) {
  if (!events.length) return <div className="empty-state">GDELT 新闻源暂未返回事件。</div>;
...
export function Signal({ data }: { data: DashboardPayload }) {
  return (
    <div className="signal">
      ...
      <p>{data.conclusion}</p>
      <small>更新时间：{new Date(data.updatedAt).toLocaleString("zh-CN", { hour12: false })}</small>
    </div>
  );
}
...
export function PredictionTable({ data }: { data: DashboardPayload }) {
  return (
    <table className="prediction">
      <thead><tr><th>周期</th><th>方向</th><th>影响评分</th><th>置信度</th><th>概率分布</th></tr></thead>
      <tbody>
